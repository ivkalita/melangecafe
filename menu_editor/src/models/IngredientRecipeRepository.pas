...
class function TIngredientRecipeRepository.GetAllForIngredient(
	ABaseName: string; AIngredientId: integer): TIngredientRecipeArray;
var
	adc: TDBCompsAddit;
begin
	...
	begin
		SQL.Text :=
			...
			'FROM ' +
				'ingredient_recipes r INNER JOIN ingredients i ON r.part_id = i.id ' +
				'INNER JOIN measures m ON i.measure_id = m.id ' +
			'WHERE ' +
				'r.src_id = :src_id AND r.expiredAt is NULL ' +
			'ORDER BY r.id';
		ParamByName('src_id').AsInteger := AIngredientId;
		try
			Open;
		except
			on E:Exception do
		...
		begin
			...
			result[high(result)] := TIngredientRecipe.Create(
				FieldByName('r_id').AsInteger,
				TIngredient.Create(AIngredientId),
				TIngredient.Create(
