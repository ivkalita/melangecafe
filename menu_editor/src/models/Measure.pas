...
begin
	...
	begin
		result := false;
		raise Exception.Create('TMeasure.Update(); Can not init adc');
		adc.Destroy;
		Exit;
	end;
	with adc.query do
	begin
		SQL.Text := 'UPDATE measures SET caption = :caption WHERE id = :id;';
		ParamByName('caption').AsString := Caption;
		ParamByName('id').AsInteger := Id;
		try
			ExecSQL();
			...
			begin
				result := false;
				raise Exception.Create(
					'TMeasure.Update(); Can not execute insert query. Error: ' + E.Message
				);
				adc.Destroy;
				Exit;
            end;
